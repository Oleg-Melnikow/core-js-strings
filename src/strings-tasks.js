...
function isPalindrome(str) {
  ...
  function convert(l) {
    return l.split(' ').join('').split(',').join('').toLocaleLowerCase();
  }
...
function encodeToRot13(str) {
  const normalOrder = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
  const rot13Order = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  const arr = str.split('');
  return arr
    .map((el) => {
      return normalOrder.includes(el)
        ? rot13Order[normalOrder.indexOf(el)]
        : el;
    })
    .join('');
  // throw new Error('Not implemented');
